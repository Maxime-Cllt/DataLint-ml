...
def run_inference(
        ...
        for i, text in enumerate(texts):
            ...
            try:
                ...
                if model.config.num_labels == 1:
                    ...
                    logger.error(
                        f"Unexpected number of labels: {model.config.num_labels}"
                    )
                    raise ValueError(
                        f"Model has {model.config.num_labels} labels, expected 1 or 2"
                    )
    ...
    for pred, count in pred_distribution.items():
        ...
        logger.info(
            f"  - Predicted {pred} ({safety_label}): {count} samples ({count / len(predictions) * 100:.1f}%)"
        )
...
def preprocess_data(dataset: pd.DataFrame) -> Tuple[List[str], List[int]]:
    ...
    if len(unique_targets) != 2 or not all(t in [0, 1] for t in unique_targets):
        logger.warning(
            f"Expected binary targets [0, 1], but found: {sorted(unique_targets)}"
        )
    ...
    truncated_count = sum(
        1
        for orig, trunc in zip(original_lengths, [len(t) for t in input_texts])
        if orig > trunc
    )
    ...
    logger.info(
        f"  - Average final length: {np.mean([len(t) for t in input_texts]):.2f}"
    )
...
def save_results(
    ...
    results_dir = os.path.join(
        get_project_root(), "results", "evaluation", model_version
    )
    ...
    axes[0, 1].hist(
        probabilities, bins=50, alpha=0.7, color="skyblue", edgecolor="black"
    )
    axes[0, 1].axvline(
        x=metrics["threshold"],
        color="red",
