...
def visualize_embeddings_tsne_filtered(model, tokenizer, num_tokens=200):
    ...
    plt.close()
    print(
        "Filtered t-SNE embedding visualization saved as tsne_embeddings_filtered.png"
    )
...
def calculate_comprehensive_metrics(
    ...
    try:
        ...
        metrics["precision_unsafe"] = precision_score(
            true_labels, predictions, pos_label=1, zero_division=0
        )
        metrics["recall_unsafe"] = recall_score(
            true_labels, predictions, pos_label=1, zero_division=0
        )
        metrics["f1_unsafe"] = f1_score(
            true_labels, predictions, pos_label=1, zero_division=0
        )
    ...
    if len(unique_true) == 1:
        logger.warning(
            f"WARNING: All true labels are the same ({list(unique_true)[0]})"
        )
